...
proc lexCode*(code: string): Deque[LexNode] =
  var pieces: Deque[LexNode]
  ...
  proc digestBuffer(): void =
    if buffer.len > 0:
      pieces.addLast LexNode(kind: lexToken, text: buffer, line: line, column: column)
      buffer = ""
  ...
  proc digestIdentation(): void =
    ...
    if level > 0:
      for i in 1..level.int:
        pieces.addLast LexNode(kind: lexControl, operator: controlIndent, line: line, column: column)
    elif level < 0:
      for i in 1..(-level.int):
        pieces.addLast LexNode(kind: lexControl, operator: controlOutdent, line: line, column: column)
  ...
  for c in code:
    ...
    of lexStateIndent:
      ...
      of '(':
        digestIdentation()
        pieces.addLast LexNode(kind: lexControl, operator: controlParenOpen, line: line, column: column)
    ...
    of lexStateSpace:
      ...
      of '(':
        ...
        lexingState = lexStateSpace
      of ')':
        pieces.addLast LexNode(kind: lexControl, operator: controlParenClose, line: line, column: column)
        digestBuffer()
        lexingState = lexStateSpace
      else:
        buffer = $c
